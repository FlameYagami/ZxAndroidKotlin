...
import kotlinx.android.synthetic.main.include_deck.*
...
class DeckEditorActivity : BaseBindingActivity() {
    ...
    private var mUgDeckAdapter = DeckAdapter(this)
    private var mEgDeckAdapter = DeckAdapter(this)
    private var bannerPageChangeListener = BannerExView.PageListener()
...
//        fabSearch.onClick { onSearch_Click() }
    ...
    private fun addCard(data: List<*>, position: Int) {
        val cardBean = data[position] as CardBean
        val numberEx = CardUtils.getNumberEx(cardBean.image, bannerPageChangeListener.getCurrentIndex())
        val imagePath = PathManager.pictureDir + File.separator + numberEx + context.getString(R.string.image_extension)
        val areaType = CardUtils.getAreaType(cardBean)
    ...
    private fun updateSingleRecyclerView(areaType: Enum.AreaType, operateType: Enum.OperateType, position: Int) {
        ...
        if (operateType == Enum.OperateType.Add) {
            if (areaType == Enum.AreaType.Player) {
                Glide.with(this).load(PathManager.pictureDir + File.separator + mDeckManager.playerList[0].numberEx + context.getString(R.string.image_extension).toString())
    ...
    private fun updateAllRecyclerView() {
        if (mDeckManager.playerList.isNotEmpty()) {
            Glide.with(this).load(PathManager.pictureDir + File.separator + mDeckManager.playerList[0].numberEx + context.getString(R.string.image_extension)).error(null).into(imgPl)
        }
