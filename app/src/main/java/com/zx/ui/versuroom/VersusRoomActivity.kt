...
class VersusRoomActivity : BaseExActivity(), AppBarView.NavigationClickListener {
    ...
    internal var viewContent: LinearLayout? = null
    @BindView(R.id.tv_room_id)
    internal var tvRoomId: TextView? = null
    ...
    override fun initViewAndData() {
        ...
        // 选手进入房间事件注册
        RxBus.instance.addSubscription(this, RxBus.instance.toObservable(EnterGameEvent::class.java).subscribe({ this.onEnterRoom(it) }))
        // 选手状态事件注册
        RxBus.instance.addSubscription(this, RxBus.instance.toObservable(DuelistStateEvent::class.java).subscribe({ this.onDuelistState(it) }))
        // 离开房间事件注册
        RxBus.instance.addSubscription(this, RxBus.instance.toObservable(LeaveGameEvent::class.java).observeOn(AndroidSchedulers.mainThread()).subscribe({ this.onLeaveRoom(it) }))
        // 开始游戏事件注册
        RxBus.instance.addSubscription(this, RxBus.instance.toObservable(StartGameEvent::class.java).subscribe({ this.onStartGame(it) }))

        ownerType = MyApp.Client?.Player?.type as Byte
        tvRoomId?.text = (MyApp.Client as Client).Room?.roomId
    }
...
//        }
    ...
    private inner class UISubscriber internal constructor() {
        ...
        init {
            stopMePlease = Observable.interval(100, TimeUnit.MILLISECONDS).observeOn(AndroidSchedulers.mainThread()).subscribe { aLong -> updateUI() }
        }
