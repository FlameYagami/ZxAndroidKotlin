...
import com.zx.game.utils.CardUtils.isLife
import com.zx.game.utils.CardUtils.isVoid
...
(// 卡组名称
    ...
    private fun loadDeck() {
        for (numberEx in NumberExList) {
            ...
            val imagePath = pictureCache + File.separator + numberEx + context?.getString(R.string.image_extension)
    ...
    private fun orderByValue(deckList: MutableList<DeckBean>) {
        val tempDeckList = deckList.sortWith(compareBy({ it.camp }, { it.cost }, { it.power }, { it.numberEx })) as ArrayList<DeckBean>
        deckList.clear()
