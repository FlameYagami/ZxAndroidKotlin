...
object DeckUtils {
    ...
    val deckNameList: List<String>
        get() {
            ...
            deckNameList.addAll(FileUtils.getFileNameExList(deckPath)
    ...
    val deckPreviewList: List<DeckPreviewBean>
        get() {
            ...
            deckPathList.addAll(FileUtils.getFilePathList(deckPath).filter { deckPath -> deckPath.endsWith(context?.getString(R.string.deck_extension).toString()) }.toList())
            for (deckPath in deckPathList) {
                ...
                val statusMain = context?.getString(if (getMainCount(cardBeanList) == 50) R.string.deck_complete else R.string.deck_not_complete).toString()
                val statusExtra = context?.getString(if (getExtraCount(cardBeanList) == 10) R.string.deck_complete else R.string.deck_not_complete).toString()
    ...
    private fun getPlayerPathList(cardBeanList: List<CardBean>): List<String> {
        ...
        pathList.addAll(cardBeanList
                .filter { cardBean -> getAreaType(cardBean) == Enum.AreaType.Player }
                .map { bean -> PathManager.pictureCache + File.separator + bean.number + context?.getString(R.string.image_extension) })
        return pathList
    }
